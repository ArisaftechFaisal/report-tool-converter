...
use crate::constants;
use std::str::FromStr;
use crate::converter::field::{Field, FieldVariant, InputSpecification};
...
fn parse(path: &str) -> Result<Vec<Field>> {
    ...
    let mut max_field_cnt = 100usize;
    ...
    'field_loop: for field_index in 1..max_field_cnt {
        ...
        'row_loop: for row in rows {
            ...
            let col_index = if field_index == 1 {1} else {(field_index - 1) * 2 + 1};
            ...
            match subject {
                Subject::Required => {
                    ...
                    match has_required {
                        Some(b) => {is_required = b;},
                        None => { break 'field_loop; }
                    }
